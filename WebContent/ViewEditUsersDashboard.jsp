...
<%@ include file = "functions.jsp" %>
	...
	<body>
		...
		<br>
		<div>
			<form method="post" action="temp.jsp"> <!-- Change this to the correct page and error will go away -->
					<%
					try{
					...
					<div>
						<label>Old Password:</label> <input id = "field" type="password" name = "password">
					</div>
					...
					<div>
						<label>State:(two capital letters)</label><input id = "field" type="text" pattern='[A-Z]{2}' name='state'required value=<%out.print("'"+session.getAttribute("state")+"'");%>/>
					</div> 
					<div>
